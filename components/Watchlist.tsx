...
import { Trash2, ExternalLink, TrendingUp, TrendingDown, Minus, RefreshCw } from 'lucide-react';
...
const Watchlist: React.FC<WatchlistProps> = ({ entries, onRemove, onSelect, onRefreshAll, isRefreshing }) => {
    ...
    <div className="mt-12 glass-card rounded-3xl p-6 border-slate-700/50 shadow-2xl overflow-hidden relative">
      <div className="flex justify-between items-center mb-6">
        ...
        <button 
          ...
          className="flex items-center gap-2 bg-slate-800 hover:bg-slate-700 text-slate-300 px-4 py-2 rounded-xl text-xs font-bold transition-all disabled:opacity-50"
        >
        ...
        {entries.map((entry) => (
          ...
          >
            ...
            <div className="flex justify-between items-start mb-3">
              <div>
                <h4 className="text-white font-bold">{entry.companyName}</h4>
      ...
      <div className="mt-6 flex items-center gap-2 p-3 bg-blue-500/10 border border-blue-500/20 rounded-xl text-[11px] text-blue-300">
        ...
        <span>L'AI monitora periodicamente queste azioni per notificarti cambiamenti di segnale sul lungo periodo.</span>
      </div>
    </div>
  );
};
